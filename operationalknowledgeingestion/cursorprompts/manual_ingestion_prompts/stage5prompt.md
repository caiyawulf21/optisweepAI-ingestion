...
For multi-source ingestion, do not treat these examples as required candidates for every source.
A candidate should be created only when the current source contains direct evidence for it.
Do not backfill OptiSweep/manual-specific examples into training videos, incidents, SME notes, or non-OptiSweep documents unless that source explicitly supports the candidate.
...
If the source says â€œmaintenance personnel,â€ map based on risk:
...
```json
{
  ...
  "input_operational_context": "data/output/manual_optisweep_om_v3/stage_4_operational_context/operational_context.json",
  "candidate_count": 0,
...
````markdown
## Current Stage: Stage 5 â€” LLM Runbook Candidate Discovery
...
- Output contains roughly 15â€“35 useful runbook candidates.
