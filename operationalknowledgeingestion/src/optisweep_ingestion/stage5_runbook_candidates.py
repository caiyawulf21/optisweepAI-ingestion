...
from operationalknowledgeingestion.src.optisweep_ingestion.services.id_generator import make_candidate_id
...
from operationalknowledgeingestion.src.optisweep_ingestion.tools.artifact_linker import IMAGE_NOTE_MARKER, enrich_runbook_candidates
...
PRIORITY_REPORT_KEYS = {
    "Start OptiSweep System": "start_optisweep_system",
    "Stop OptiSweep System": "stop_optisweep_system",
    "Start Operator Station": "start_operator_station",
    "Stop Operator Station": "stop_operator_station",
    "Check Tipper Heartbeat Stats": "check_tipper_heartbeat_stats",
    "Check Operator Station Alarms": "check_operator_station_alarms",
    "Review VISU_IO_DIAG Screen": "review_visu_io_diag_screen",
    "Use Hospital Add Tote Screen": "use_hospital_add_tote",
    "Use Hospital Remove Tote Screen": "use_hospital_remove_tote",
    "Recover AGV Bump Fault": "recover_agv_bump_fault",
    "Recover Faulted AGV With Tote": "recover_faulted_agv_with_tote",
    "Initialize Tote With Sorter Scanner": "initialize_tote_with_sorter_scanner",
}

PRIORITY_EVIDENCE_TERMS = {
    "Start OptiSweep System": [["start optisweep"], ["start the system"], ["starting the system"]],
    "Stop OptiSweep System": [["stop optisweep"], ["stop the system"], ["stopping the system"]],
    "Start Operator Station": [["start operator station"], ["starting operator station"]],
    "Stop Operator Station": [["stop operator station"], ["stopping operator station"]],
    "Check Tipper Heartbeat Stats": [["heartbeat"]],
    "Check Operator Station Alarms": [["operator station alarm"], ["operator station alarms"], ["alarm history"]],
    "Review VISU_IO_DIAG Screen": [["visu_io_diag"], ["visu io diag"]],
    "Use Hospital Add Tote Screen": [["add tote"]],
    "Use Hospital Remove Tote Screen": [["remove tote"]],
    "Recover AGV Bump Fault": [["agv", "bump"]],
    "Recover Faulted AGV With Tote": [["faulted agv", "tote"], ["agv", "fault", "tote"]],
    "Initialize Tote With Sorter Scanner": [["sorter", "scanner", "tote"]],
}
...
class AzureOpenAIRunbookCandidateClient:
    def __init__(
        ...
        default_prompt = Path(__file__).resolve().parents[1] / "prompts" / "stage5_runbook_candidate_discovery_prompt.md"
...
def extract_runbook_candidates(
    ...
    if isinstance(llm_client, AzureOpenAIRunbookCandidateClient):
        backfill_prompt_path = Path(__file__).resolve().parents[1] / "prompts" / "stage5_runbook_candidate_backfill_prompt.md"
    ...
    selected, artifact_link_notes = enrich_runbook_candidates(selected, artifacts, contexts)
    warnings.extend(artifact_link_notes)
    post_enrichment_dropped = 0
    validated_selected: list[dict[str, Any]] = []
    for candidate in selected:
        errors = validate_candidate_record(candidate, known_artifact_ids, known_context_ids)
        if errors:
            post_enrichment_dropped += 1
            warnings.append(
                f"Dropped candidate '{candidate.get('candidate_id', '?')}' after artifact linking: {'; '.join(errors)}"
            )
        else:
            validated_selected.append(candidate)
    if post_enrichment_dropped:
        dropped += post_enrichment_dropped
    ...
    report = build_extraction_report(
        ...
        priority_evidence_text=build_priority_evidence_text(bundle, artifacts, contexts),
    )
        ...
        packets.append(
            {
                ...
                "source_refs": _section_source_refs(section, lineage, page_start, page_end, section_id),
            }
        )
    return packets
...
def _section_source_refs(
    ...
    refs = [ref for ref in section.get("source_refs", []) if isinstance(ref, dict)]
    if refs:
        return refs
    return [
        build_source_ref(
...
def ensure_priority_candidates(
    ...
    """Legacy deterministic repair helper retained for backward-compatible tests.

    The production extraction path intentionally does not call this. Priority
    candidates should be discovered from source evidence, not synthesized from a
    manual-specific checklist across unrelated sources.
    """
...
def build_coverage_report(
    ...
    return {
        "source_procedure_inventory_count": total,
...
def build_extraction_report(
    ...
    priority_evidence_text: str | None = None,
) -> dict[str, Any]:
    ...
    priority_check = build_priority_candidate_check(candidates, priority_evidence_text=priority_evidence_text)
    return {
        "source_id": lineage.source_id,
        "source_type": lineage.source_type,
        "ingestion_batch_id": lineage.ingestion_batch_id,
        ...
        "candidates_with_image_notes": sum(
            1
            for c in candidates
            if any(IMAGE_NOTE_MARKER in str(step) for step in (c.get("rough_steps") or []))
        ),
        ...
        "priority_candidate_check": priority_check,
        "applicable_priority_candidates": [
            title
            for title, key in PRIORITY_REPORT_KEYS.items()
            if priority_check.get(key, {}).get("applicable")
        ],
        "missing_priority_candidates": build_missing_priority_candidates(
            candidates,
            priority_evidence_text=priority_evidence_text,
        ),
...
def build_priority_candidate_check(
    candidates: list[dict[str, Any]],
    priority_evidence_text: str | None = None,
) -> dict[str, Any]:
    checks = {}
    for title, keywords in REQUIRED_PRIORITY_CANDIDATES.items():
        key = PRIORITY_REPORT_KEYS[title]
        applicable = _priority_applicable(title, priority_evidence_text)
        checks[key] = _candidate_check(candidates, keywords) | {
            "applicable": applicable,
            "missing_when_applicable": applicable and _find_candidate(candidates, keywords) is None,
        }
...
def build_missing_priority_candidates(
    candidates: list[dict[str, Any]],
    priority_evidence_text: str | None = None,
) -> list[str]:
    missing: list[str] = []
    for title, keywords in REQUIRED_PRIORITY_CANDIDATES.items():
        if not _priority_applicable(title, priority_evidence_text):
            continue
...
def build_priority_evidence_text(
    bundle: dict[str, Any],
    artifacts: list[dict[str, Any]],
    contexts: list[dict[str, Any]],
) -> str:
    ...
    for section in bundle.get("sections") or []:
        ...
        parts.append(str(section.get("text_preview") or ""))
    for artifact in artifacts:
        parts.extend(
            [
                str(artifact.get("title") or ""),
                str(artifact.get("caption_text") or ""),
                str(artifact.get("nearby_text") or ""),
                str(artifact.get("ocr_text") or ""),
                str(artifact.get("retrieval_text") or ""),
                " ".join(str(tag) for tag in artifact.get("tags") or []),
            ]
        )
    for context in contexts:
        parts.extend(
            [
                ...
                str(context.get("details") or ""),
                str(context.get("retrieval_text") or ""),
                " ".join(str(term) for term in context.get("key_terms") or []),
            ]
        )
    return " ".join(parts).lower()


def _priority_applicable(title: str, priority_evidence_text: str | None) -> bool:
    if priority_evidence_text is None:
        return True
    evidence = priority_evidence_text.lower()
    groups = PRIORITY_EVIDENCE_TERMS.get(title) or [REQUIRED_PRIORITY_CANDIDATES[title]]
    return any(all(term.lower() in evidence for term in group) for group in groups)
