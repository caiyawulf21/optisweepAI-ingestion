...
Do not invent visual details, screenshots, page refs, figure refs, or section refs that are not supported by the caption, nearby text, OCR text, or retrieval text.
For training video artifacts, treat `nearby_text` as the aligned transcript and `ocr_text` as locally extracted slide text. Prefer source-grounded synthesis of both, and note low OCR confidence or unclear transcript alignment in `enrichment_notes`.
