...
from operationalknowledgeingestion.src.optisweep_ingestion.stage4_operational_context import validate_context_record
from operationalknowledgeingestion.src.optisweep_ingestion.stage5_runbook_candidates import validate_candidate_record
...
def main(
    ...
    def _context_match_metrics(cands: list[dict], ctx_ids: set[str]) -> dict[str, int]:
        refs: set[str] = set()
        for cand in cands:
            refs.update(cand.get("related_context_ids") or [])
        matched = refs & ctx_ids
        return {
            "referenced_context_ids": len(refs),
            "matched_context_ids": len(matched),
            "unmatched_context_ids": len(refs - ctx_ids),
        }

    before_context_match = _context_match_metrics(candidates, known_context_ids)
    ...
    after_context_match = _context_match_metrics(validated_candidates, set(context_index))
    ...
    candidate_report.update(
        {
            ...
            "before_context_id_match_metrics": before_context_match,
            "after_context_id_match_metrics": after_context_match,
    ...
    typer.echo("Context ID match before: " + str(before_context_match))
    typer.echo("Context ID match after:  " + str(after_context_match))
