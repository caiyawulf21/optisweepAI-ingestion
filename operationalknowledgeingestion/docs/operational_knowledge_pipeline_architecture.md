...
## Decision: Video Plus VTT Is A Training Source Bundle

For training recordings, the first supported v1 input shape is a video file plus an external `.vtt` transcript.

The video is the timeline authority because the transcript does not identify slide changes. The training video builder detects slide intervals from cropped video frames, captures the shared-slide region as source artifacts, and aligns VTT cues by timestamp overlap. It emits normalized slide segment records that downstream extraction stages can consume.

The v1 builder does not create operational context, runbook candidates, canonical runbooks, relationship links, embeddings, or graph records. Those remain downstream stages.

Known-layout screen-share recordings should use an explicit crop box. Local Tesseract OCR can be used to detect slide title/text changes and improve segmentation. Automatic meeting-layout detection, embedded-caption extraction, bulk LLM vision segmentation, and video-only transcription are future enhancements.

Training video records should pass through a deterministic preparation step before semantic extraction. That step converts slide segments into source-bundle pages and sections, attaches video timestamp source references, and prepares frame artifacts with section IDs. The existing Stage 4 operational context extractor and Stage 5 runbook candidate extractor should then be reused.

Training video OCR is additive. It should be stored with backend, timestamp, source image path, confidence when available, and text hash. It should help segmentation and retrieval preparation, but transcript timestamps and video frame timestamps remain the lineage authority.
